...
$tableName = Get-AzStorageTable –Context $ctx | select -ExpandProperty Name
$storageTable = Get-AzStorageTable –Name $tableName –Context $ctx

$cloudTable = (Get-AzStorageTable –Name $tableName –Context $ctx).CloudTable

Import-Csv "C:\scripts\strava-1-1\AzurePSFunction\Strava\output\stravafew.csv"  -Delimiter ";" |ForEach-Object {
    Clear-Variable var*
    ...
    $VAR_average_speed1 =[convert-decia] $_.average_speed -replace ",", '.' 
    $VAR_average_speed = [decimal]$VAR_average_speed1*3.6
...
Write-Host "rit naam = $var_name met de snelheid [decimal]$VAR_average_speed "
# add four rows 
<# Add-AzTableRow `
    -table $cloudTable `
...
Get-AzTableRow -Table $cloudTable -PartitionKey ride

Get-AzTableRow -Table $cloudTable -ColumnName "resource_state" -Value "2" -Operator Equal | select kilojoules
